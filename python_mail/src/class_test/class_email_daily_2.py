...
class FileWR(OracleExecution):
    ...
    def file_write_f(self, message_date, job_flag, sleep_seconds=0.04):
        try:
...
if __name__ == '__main__':
    # print(sys.path)
    # 实例化信息&检索语句初始化
    confJob = sql_conf.conf_JOB
    # 文件生成路径
    filePath = bas_mail_conf.mail_file_path_class
    # 各报表标题初始化
    fileTitleJob = bas_mail_conf.file_title_job

    # Start
    t1 = threading.Thread(target=ora_job, args=(confJob, filePath, fileTitleJob, ))
    # ora_job(confJob, filePath, fileTitleJob)
    t1.start()
    t1.join()

    # confAll = [sql_conf.conf_JOB, sql_conf.conf_PKG, sql_conf.conf_GATHER]
    # num_loops = range(len(confAll))
    # print(num_loops)
