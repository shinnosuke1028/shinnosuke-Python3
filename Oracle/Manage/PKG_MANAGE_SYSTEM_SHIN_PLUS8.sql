...
-----------------------------------------------------------------------------------------------
  PROCEDURE PROC_PARTITION_CLEANUP_RANGE(V_TBNAME VARCHAR2, V_TM_GRN VARCHAR2, V_DATE_THRESHOLD_START VARCHAR2, V_DATE_THRESHOLD_END VARCHAR2) IS
   ...
   -- v_date_name varchar2(200);--分区扩展时间段SQL拼接
    ...
    where table_name = V_TBNAME and (partition_name not like 'P2%' and partition_name <> 'HR01')--SYS_21313
    ...
    BEGIN
     ...
     select count(distinct table_name) into v_tb_partition_exists_flag from user_tables where table_name  = V_TBNAME;
      ...
      (
          ...
          from USER_TAB_PARTITIONS t
          /* using (table_name) */
          where (partition_name not like 'P2%' and partition_name <> 'HR01') and table_name = V_TBNAME
      );
     ...
     select count(distinct table_name) into v_tb_partition_exists_flag from user_tables where table_name  = V_TBNAME;
      ...
      while v_date >= to_date(V_DATE_THRESHOLD_START,'yyyymmdd') and v_date <= to_date(V_DATE_THRESHOLD_END,'yyyymmdd') LOOP
        ...
        if v_partition_type = 'SHIN'
          ...
          then
            PKG_MANAGE_SYSTEM_SHIN_PLUS8.PROC_PARTITION_LOCATE(V_TBNAME, to_char(v_date, 'yyyymmdd'), V_PARTITION_NAME);
            if V_PARTITION_NAME <> 'NULL'
              then
                EXECUTE IMMEDIATE 'ALTER TABLE '||V_TBNAME||' TRUNCATE PARTITION('||V_PARTITION_NAME||')';
                dbms_output.put_line('表 '||V_TBNAME||' 下的SYS自动分区 '||V_PARTITION_NAME||' 已清理！');--执行SQL后输出标记
                v_loop_log := v_loop_log + 1;

                if V_TM_GRN = 2 then 
                  v_date := v_date + 1;
                elsif V_TM_GRN = 3 then 
                  v_date := v_date + 7;
                elsif V_TM_GRN = 4 then 
  ...
  PROCEDURE PROC_PARTITION_TRUNCATE_RANGE_INDEX(V_TBNAME VARCHAR2, V_TM_GRN VARCHAR2, V_DATE_THRESHOLD_START VARCHAR2, V_DATE_THRESHOLD_END VARCHAR2) IS
    ...
    BEGIN
     ...
     select count(distinct table_name) into v_tb_partition_exists_flag from user_tables where table_name  = V_TBNAME;
      ...
      if V_PARTITION_NAME = 'NULL'
         then 
             dbms_output.put_line('表 '||V_TBNAME||' 下对应时间 '||V_DATE_THRESHOLD_START||' 的分区不存在！完成时间戳：'||to_char(sysdate,'yyyy/mm/dd hh24:mi:ss')||'.');
             return;
      end if;
      ...
      --现有：2019/06/19（天） /2019/06/17（周） /2019/06/01（月）
      v_date := to_date(V_DATE_THRESHOLD_START, 'yyyymmdd');--起始时间戳'yyyymmdd'格式化--2019/06/20

      while v_date >= to_date(V_DATE_THRESHOLD_START, 'yyyymmdd') and v_date <= to_date(V_DATE_THRESHOLD_END,'yyyymmdd') LOOP
      exit when v_date > to_date(V_DATE_THRESHOLD_END, 'yyyymmdd');--超界退出
        if v_partition_type = 'SHIN'
          then
            --v_loop_log := 1;
            if  V_TM_GRN = '2' --天级分区清理
          ...
          then
            ...
            if V_PARTITION_NAME <> 'NULL'
              then
                EXECUTE IMMEDIATE 'ALTER TABLE '||V_TBNAME||' TRUNCATE PARTITION('||V_PARTITION_NAME||') UPDATE GLOBAL INDEXES';
                dbms_output.put_line('表 '||V_TBNAME||' 下的SYS自动分区 '||V_PARTITION_NAME||' 已清理！');--执行SQL后输出标记
                v_loop_log := v_loop_log + 1;
    ...
    BEGIN
        ...
        while v_rownum < v_rownum_max loop--******
            ...
            if v_tb_exists_flag = 0
               ...
               if V_TBNAME = 'ZYJ_MR_USER_CELL'
                   then
                       execute immediate 'select to_char(sysdate, ''yyyymm'')||t.day_range from FAST_DATA_PROCESS_TEMPLATE t where t.table_name = :table_name'
                       into v_date_start_noclean_vc using V_TBNAME;
                       if v_date_start_clean_vc = v_date_start_noclean_vc
                           then
                               DBMS_STATS.GATHER_TABLE_STATS ('LRNOP','ZYJ_MR_USER_CELL');
                               dbms_output.put_line('时间戳：'||v_date_start_clean_vc||', 表 '||V_TBNAME||' 的周期清理跳过！');
                               continue;
                       end if;
               --20191125  CHECK_RESULT_2G4G每月保留单日数据（目标时间戳：18th）
               elsif V_TBNAME = 'CHECK_RESULT_2G4G'
                   then 
                       execute immediate 'select to_char(sysdate, ''yyyymm'')||t.day_range from FAST_DATA_PROCESS_TEMPLATE t where t.table_name = :table_name'
                       into v_date_start_noclean_vc using V_TBNAME; 
                       ...
                       if v_date_start_clean_vc = v_date_start_noclean_vc
                           then
                               --DBMS_STATS.GATHER_TABLE_STATS ('LRNOP','CHECK_RESULT_2G4G');
                               dbms_output.put_line('时间戳：'||v_date_start_clean_vc||', 表 '||V_TBNAME||' 的周期清理跳过！');
                               continue;
                       end if;
               end if;
               ...
               (
                 ...
                 from
                 (
                    select decode(count(partition_name), 0, ''NULL'', max(partition_name)) partition_name from
                    (select * from FAST_DATA_PROCESS_TEMPLATE s where s.table_name = :V_TBNAME)s
                    left join USER_TAB_PARTITIONS t
                    using (table_name)
                    where (partition_name not like ''P2%''
                    and partition_name <> ''HR01'')
                    --规避分区头判断结果为NULL的情况
                 )
...
-----------------------------------------------------------------------------------------------
  PROCEDURE PROC_PARTITION_LOCATE(V_TABLE_NAME VARCHAR2, V_DATE_THRESHOLD_START VARCHAR2, V_PARTITION_NAME OUT VARCHAR2) IS
    ...
    CURSOR CUR_PARTITION is --字典表内获取非标准分区的分区名
    select table_name,t.partition_name,t.high_value from USER_TAB_PARTITIONS t where table_name = V_TABLE_NAME 
    and (partition_name not like 'P2%' and partition_name <> 'HR01')
    order by to_number(substr(partition_name,6)) desc;--按照分区名降序遍历

    

    BEGIN
        execute immediate 'select tm_grn from FAST_DATA_PROCESS_TEMPLATE where table_name = :V_TABLE_NAME 'into v_tm_grn using V_TABLE_NAME;
        OPEN CUR_PARTITION; --开始索引字典表
        LOOP--------------
            ...
            LOOP
                ...
                if substr(NESTED_TAB(I).high_value, 1, 2) = 'TI'
                  then 
                    v_high_value_vc := substr(NESTED_TAB(I).high_value, 12, 10);
                else 
                ...
                if v_tm_grn = 2
                  then
                    v_date_target := to_char(to_date(v_high_value_vc,'yyyy-mm-dd')-1, 'yyyymmdd');
                --9
                elsif v_tm_grn = 3
                  then
                    v_date_target := to_char(to_date(v_high_value_vc,'yyyy-mm-dd')-7, 'yyyymmdd');
                --A
                elsif v_tm_grn = 4
                  then
                    v_date_target := to_char((to_date(v_high_value_vc,'yyyy-mm-dd') - interval '1' MONTH), 'yyyymmdd');
                end if;

                if (v_date_target = V_DATE_THRESHOLD_START)
                then
                    V_PARTITION_NAME := NESTED_TAB(I).partition_name;
                    --dbms_output.put_line('目标分区名：'||V_PARTITION_NAME);
                    return;--降序遍历，达到目标分区值时(指定时间&指定时间一周前)，游标结束，退出！！！抛弃多余遍历
                end if;
...
-----------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------
  PROCEDURE SQL_GET_DDL_WK(I_FROM_TABLENAME VARCHAR2,  I_TM_GRN VARCHAR2,  I_FROM_OWNER VARCHAR2, I_TABLESPACE VARCHAR2, I_DATE_THRESHOLD_START VARCHAR2, I_ENABLE_SYS NUMBER:=0) is
      V_DATE  date;
      ...
      cursor CUR_SQL_1 is
      select 'comment on column WK_'||table_name||'.'||column_name ||' is '''||comments||'''' sql_string from 
      (
        select TABLE_NAME, COLUMN_NAME, COMMENTS from user_col_comments t where t.table_name = I_FROM_TABLENAME
      );
      cursor CUR_SQL_2 is
      select 'comment on table WK_'||table_name||' is '''||comments||'''' sql_string from 
      (
        select TABLE_NAME, COMMENTS from user_tab_comments t where t.table_name = I_FROM_TABLENAME
      );
      cursor CUR_SQL_3 is
      select distinct table_name_type, search_condition_vc index_name, index_vc, uniqueness
      from
      (
      ...
      ') within GROUP(order by search_condition_vc) search_condition_vc--换行符分隔
            ...
            --select *
            from user_constraints u where u.table_name = I_FROM_TABLENAME
          ...
          from user_indexes t where t.table_name = I_FROM_TABLENAME
      )x
      --group by table_name
      left join
      (
          ...
          where table_name = I_FROM_TABLENAME
      ...
      C_ROW_1 CUR_SQL_1%rowtype;--定义行对象
      C_ROW_2 CUR_SQL_2%rowtype;--定义行对象
      
      TYPE INDEX_TYPE IS RECORD(
          TABLE_NAME_TYPE VARCHAR2(100),
          INDEX_NAME VARCHAR2(50),
          INDEX_VC VARCHAR2(500),
          UNIQUENESS VARCHAR2(30)
      );
      --TYPE INDEX_TYPE IS TABLE OF user_indexes%ROWTYPE INDEX BY BINARY_INTEGER;
      C_INDEX_TAB INDEX_TYPE; 
      ...
      BEGIN
        ...
        if I_ENABLE_SYS = 0 
            ...
            V_PART_NAME := 'HR01';--自增分区首分区名，固定
            V_DATE := trunc(to_date(I_DATE_THRESHOLD_START,'yyyymmdd'), 'mm');
            if I_TM_GRN = '2'
                then
                V_DATE_THRESHOLD_PART := to_char(V_DATE, 'YYYY-MM-DD');--2019-07-03
                V_COB_TBSP_PARTITIONSQL :=
                'PARTITION BY RANGE ('||V_DATE_FIELD||') INTERVAL (NUMTODSINTERVAL(1, ''day''))
                (
                    PARTITION '||V_PART_NAME||' VALUES LESS THAN (TO_DATE('''||V_DATE_THRESHOLD_PART||' 00:00:00'', ''SYYYY-MM-DD HH24:MI:SS'', ''NLS_CALENDAR=GREGORIAN''))
                    TABLESPACE '||I_TABLESPACE||'
                )';
                ...
                V_DATE_THRESHOLD_PART := to_char(V_DATE + 7,'YYYY-MM-DD');--2019-07-08
                V_COB_TBSP_PARTITIONSQL :=
                'PARTITION BY RANGE ('||V_DATE_FIELD||') INTERVAL (NUMTODSINTERVAL(7, ''day''))
                (
                    PARTITION '||V_PART_NAME||' VALUES LESS THAN (TO_DATE('''||V_DATE_THRESHOLD_PART||' 00:00:00'', ''SYYYY-MM-DD HH24:MI:SS'', ''NLS_CALENDAR=GREGORIAN''))
                    TABLESPACE '||I_TABLESPACE||'
                )';
                ...
                V_DATE_THRESHOLD_PART := to_char((V_DATE + interval '1' MONTH),'YYYY-MM-DD');--2019-08-01
                V_COB_TBSP_PARTITIONSQL :=
                'PARTITION BY RANGE ('||V_DATE_FIELD||') INTERVAL (NUMTOYMINTERVAL(1, ''month''))
                (
                    PARTITION '||V_PART_NAME||' VALUES LESS THAN (TO_DATE('''||V_DATE_THRESHOLD_PART||' 00:00:00'', ''SYYYY-MM-DD HH24:MI:SS'', ''NLS_CALENDAR=GREGORIAN''))
                    TABLESPACE '||I_TABLESPACE||'
                )';
        ...
        OPEN CUR_SQL_1;
            LOOP
                FETCH CUR_SQL_1 INTO C_ROW_1;
                exit when CUR_SQL_1%notfound;
                --SQL_LINE.Extend(1);
                --v_cursor:=dbms_sql.open_cursor;
                V_SSQL := C_ROW_1.SQL_STRING;
                --dbms_sql.parse(v_cursor,ssql,dbms_sql.native); --分析语句
                --v_row:=dbms_sql.execute(v_cursor); --执行语句
                --dbms_sql.close_cursor(v_cursor); --关闭光标
                execute immediate V_SSQL;
                dbms_output.put_line('SQL_LINE_'||i|| ' ：已执行！执行内容：'||V_SSQL||'.');--执行SQL后输出标记
                i := i + 1;
            END LOOP;
        CLOSE CUR_SQL_1;
        ...
        OPEN CUR_SQL_2;
            LOOP
                FETCH CUR_SQL_2 INTO C_ROW_2;
                exit when CUR_SQL_2%notfound;
                V_SSQL := C_ROW_2.SQL_STRING;
                execute immediate V_SSQL;
                dbms_output.put_line('SQL_LINE_'||i|| ' ：已执行！执行内容：'||V_SSQL||'.');--执行SQL后输出标记
                i := i + 1;
            END LOOP;
        ...
        OPEN CUR_SQL_3;
            LOOP
                FETCH CUR_SQL_3 INTO C_INDEX_TAB;
                exit when CUR_SQL_3%notfound;
                --execute immediate V_SSQL;
                dbms_output.put_line('表 '||I_FROM_TABLENAME||' 的索引信息为：'||C_INDEX_TAB.TABLE_NAME_TYPE||', 索引名为：'||C_INDEX_TAB.INDEX_NAME||' , 索引字段为：'||C_INDEX_TAB.INDEX_VC||', 索引类型：'||C_INDEX_TAB.UNIQUENESS||'. ');--执行SQL后输出标记
                i := i + 1;
            END LOOP;
        CLOSE CUR_SQL_3;
...
-----------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------
    PROCEDURE SQL_GET_DDL(I_FROM_TABLENAME VARCHAR2,  I_TM_GRN VARCHAR2,  I_FROM_OWNER VARCHAR2, I_TABLESPACE VARCHAR2, I_DATE_THRESHOLD_START VARCHAR2, I_ENABLE_SYS NUMBER:=0) is
    V_DATE  date;
    V_COB_TABLESQL CLOB;--元数据
    V_COB_TBSP_PARTITIONSQL CLOB;--表空间&分区存储
    V_COB_A2WKA varchar2(500);
